...
CREATE TABLE IF NOT EXISTS `senicare`.`customers` (
  ...
  `charger` VARCHAR(20) NOT NULL COMMENT '담당자',
  `address` TEXT NOT NULL COMMENT '고객 주소',
  `location` VARCHAR(30) NOT NULL COMMENT '지역(시,도)',
...
CREATE TABLE IF NOT EXISTS `senicare`.`tools` (
  ...
  `name` VARCHAR(255) NOT NULL COMMENT '용품명',
  `purpose` TEXT NOT NULL COMMENT '용도',
  `count` INT NOT NULL COMMENT '용품 갯수',
...
CREATE TABLE IF NOT EXISTS `senicare`.`care_records` (
  ...
  `record_date` DATE NOT NULL COMMENT '관리기록날짜',
  `contents` TEXT NOT NULL COMMENT '관리기록내용',
  `used_tool` INT NULL COMMENT '관리 기록 사용 용품',
  `count` INT NULL COMMENT '관리시 사용한 용품 개수',
  `charger` VARCHAR(20) NOT NULL COMMENT '관리기록 담당 요양사',
  `customer_number` INT NOT NULL COMMENT '관리 대상 고객',
  ...
  INDEX `cared_customer_idx` (`customer_number` ASC) VISIBLE,
...
CREATE USER 'senicare' @'%' identified by 'senicare';
